                ...
                <div class="breadcrumb-content">
                    ...
                    <div class="section-heading">
                        <h2 class="section__title">
                            ...
                            $x = 2;
                            @endphp
                            @if ($x <= count(Request::segments())) {{str_replace('-', ' '
                                ,ucfirst(Request::segment(2)))}} @else @if($breadcrumb !=null) {{$breadcrumb}} @else
                                {{str_replace('-', ' ' ,ucfirst(Request::segment(1)))}} @endif @endif </h2>
                    </div>

                    <ul class="breadcrumb__list">
                        <li class="active__list-item"><a href="{{ route('homepage') }}">@translate(Home)</a></li>
                        @for($i = 1; $i <= count(Request::segments()); $i++) <li class="bread-item">
                ...
                <div class="col-lg-12">
                    <div class="filter-bar d-flex justify-content-between align-items-center">
                        ...
                        <ul class="filter-bar-tab nav nav-tabs align-items-center" id="myTab" role="tablist">
                            ...
                            @if($courses->count() != 0)
                            <li class="nav-item"> @translate(Menampilkan) {{ $courses->firstItem() }}
                                -{{ $courses->count() < 10 ? $courses->count(): $courses->perPage() }}
                                    @translate(dari) {{ $courses->total() }} @translate(hasil)
                            </li>
                            @else
                            <li class="nav-item"> @translate(Tidak Ada Pelatihan Ditemukan)</li>
                            @endif
                        </ul>
                        ...
                        <div class="sidebar">
                            <form method="get" action="{{route('course.filter')}}" id="filter">
                                ...
                                <div class="sidebar-widget">
                                    <h3 class="widget-title">@translate(Kategori)</h3>
                                    <span class="section-divider"></span>
                                    <ul class="filter-by-category">
                                        <input value="{{$categories->first()->slug ?? ''}}" type="hidden" name="slug">
                                        @foreach(categories() as $item)
                                        <li>
                                            <div class="custom-checkbox">
                                                <input onclick="submitForm()" {{Request::get('categories')==$item->id ?
                                                'checked':null }} value="{{$item->id}}"
                                                ...
                                                <label for="chb-{{$item->id}}"
                                                    ...
                                                    <span
                                                        class="ml-2 font-size-14 primary-color-3">({{$item->courses->count()}})</span>
                                                </label>
                                            </div>
                                                ...
                                                <li class="pl-2">
                                                    <div class="custom-checkbox">
                                                        <input onclick="submitForm()"
                                                            {{Request::get('categories')==$child->id ? 'checked':null }}
                                                        value="{{$child->id}}"
                                ...
                                <div class="sidebar-widget">
                                    ...
                                    <ul class="filter-by-level">
                                        <li>
                                            <div class="custom-checkbox">
                                                <input onclick="submitForm()" {{Request::get('level')=="All Levels"
                                                    ? 'checked' :null }} value="All Levels" type="radio" id="chb19"
                                                    name="level">
                                                <label for="chb19" class="course-hvr">@translate(Semua Level)<span
                                                        class="ml-2 font-size-14 primary-color-3"></span></label>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="custom-checkbox">
                                                <input onclick="submitForm()" {{Request::get('level')=="Beginner"
                                                    ? 'checked' :null }} type="radio" id="chb20" value="Beginner"
                                                    name="level">
                                                <label for="chb20" class="course-hvr">@translate(Pemula)<span
                                                        class="ml-2 font-size-14 primary-color-3"></span></label>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="custom-checkbox">
                                                <input onclick="submitForm()" {{Request::get('level')=="Advanced"
                                                    ? 'checked' :null }} type="radio" id="chb21" value="Advanced"
                                                    name="level">
                                                <label for="chb21" class="course-hvr">@translate(Lanjutan)<span
                                                        class="ml-2 font-size-14 primary-color-3"></span></label>
                                            </div>
                                        </li>
                                    </ul>
                                ...
                                <!---Language-->
                                <div class="sidebar-widget">
                                    <h3 class="widget-title">@translate(Bahasa)</h3>
                                    <span class="section-divider"></span>
                                    <div class="sort-ordering">
                                        <select class="select2-lan selectpicker form-control w-100" name="language"
                                            ...
                                            <option value="">@translate(Semua Bahasa)</option>
                                            @foreach($languages as $item)
                                            <option value="{{$item->name}}" {{Request::get('language')==$item->name ?
                                ...
                                <!--Cast--->
                                <div class="sidebar-widget">
                                    <h3 class="widget-title">@translate(Dari Harga)</h3>
                                    <span class="section-divider"></span>
                                    <ul class="filter-by-price">
                                        <li>
                                            <div class="custom-checkbox">
                                                <input onclick="submitForm()" {{Request::get('cost')=="all" ? 'checked'
                                                    :null }} value="all" type="radio" id="chb23" name="cost">
                                                <label for="chb23">@translate(Semua)<span
                                                        class="ml-2 font-size-14 course-hvr">({{\App\Model\Course::Published()->get()->count()}})</span></label>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="custom-checkbox">
                                                <input onclick="submitForm()" {{Request::get('cost')=="free" ? 'checked'
                                                    :null }} value="free" type="radio" id="chb24" name="cost">
                                                <label for="chb24">@translate(Gratis)<span
                                                        class="ml-2 font-size-14 course-hvr">({{\App\Model\Course::Published()->where('is_free',true)->get()->count()}})</span></label>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="custom-checkbox">
                                                <input onclick="submitForm()" {{Request::get('cost')=="paid" ? 'checked'
                                                    :null }} value="paid" type="radio" id="chb25" name="cost">
                                                <label for="chb25">@translate(Berbayar)<span
                                ...
                                <!-- All instructor -->
                                <div class="sidebar-widget">
                                    <h3 class="widget-title">@translate(Dinas)</h3>
                                    <span class="section-divider"></span>
                                    <div class="sort-ordering">
                                        ...
                                        <select class="sort-ordering-select select2-instructor selectpicker"
                                            ...
                                            <option value="">@translate(Semua Dinas)</option>
                                            @foreach(\App\User::where('user_type','Instructor')->where('banned',0)->get()
                                            as $item)
                                            <option value="{{$item->id}}" {{Request::get('instructor')==$item->id ?
                                                'selected':null }}>{{$item->name}}</option>
                                            @endforeach
                                        </select>
                                            ...
                                            <div class="card-item card-preview"
                                                ...
                                                <div class="card-content">
                                                    ...
                                                    <h3 class="card__title">
                                                        <a href="{{route('course.single',$course->slug)}}"
                                                            title="{{$course->title}}">{{ Str::limit($course->title,58)
                                                            }}</a>
                                                    </h3>
                                                    <p class="card__author">
                                                        <a
                                                            href="{{route('single.instructor',$course->slug)}}">{{$course->name}}</a>
                                                    </p>
                                                        ...
                                                        <ul
                                                            ...
                                                            <li>
                                                                <span class="meta__date">
                                                                    ...
                                                                    {{$course->classes->count()}} @translate(Kelas)
                                                                </span>
                                                            </li>
                                                            <li>
                                                                <span class="meta__date">
                                                                    ...
                                                                    @endphp
                                                                    <i
                                                                        class="la la-clock-o"></i>{{duration($total_duration)}}
                                                    ...
                                                    <div
                                                        ...
                                                        <span
                                                            class="card__price"><del>{{formatPrice($course->price)}}</del></span>
                                                    ...
                                                    <div class="card-action">
                                                        <ul
                                                            ...
                                                            <li><span class="meta__date"><i
                                                                    ...
                                                                    {{$c_tooltip->classes->count()}}
                                                                    @translate(Kelas Pelatihan)</span>
                                                            </li>
                                                            <li><span class="meta__date">
                                                                    ...
                                                                    @endphp
                                                                    <i
                                                                        class="la la-clock-o"></i>{{duration($total_duration)}}</span>
                                                            </li>
                                                        </ul>
                                                    </div><!-- end card-action -->
